...
interface KeyState {
  ...
  index: number;
  exhausted: boolean;
  exhaustedAt: number;       // timestamp when it got 429
  cooldownMs: number;         // how long to wait before retrying
  totalRequests: number;
  totalErrors: number;
}

const DEFAULT_COOLDOWN = 42_000; // 42 seconds (Gemini free tier resets every ~38s)

let keys: KeyState[] = [];
let currentIndex = 0;
let initialized = false;

export function initKeyManager(keysStr: string) {
...
export function markExhausted(keyValue: string) {
