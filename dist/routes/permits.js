...
const permitNumber_1 = require("../utils/permitNumber");
const router = (0, express_1.Router)();
// ── POST /api/inventory/withdraw ──────────────────────────────────────────────
router.post("/withdraw", auth_1.requireAuth, async (req, res, next) => {
    try {
        ...
        const permitNumber = await (0, permitNumber_1.generateWithdrawalPermitNumber)();
...
router.post("/supply", auth_1.requireAuth, async (req, res, next) => {
    try {
        ...
        const permitNumber = await (0, permitNumber_1.generateSupplyPermitNumber)();
        const permit = await database_1.prisma.$transaction(async (tx) => {
            ...
            if (newProducts && newProducts.length > 0) {
                for (const np of newProducts) {
                    if (!np.name || !String(np.name).trim())
                        continue;
