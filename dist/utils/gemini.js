...
async function tryGemini(imageBase64, mimeType) {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        const apiKey = (0, keyManager_1.getNextKey)();
        ...
        try {
            ...
            if (!res.ok) {
                ...
                if (res.status === 429 || res.status === 503) {
                    (0, keyManager_1.markExhausted)(apiKey);
            ...
            if (err.message?.startsWith("Gemini API:"))
                throw err;
...
function parseAIResponse(text) {
    ...
    if (Array.isArray(parsed)) {
        return {
            items: parsed.filter((it) => it.name).map((it) => ({
    ...
    return {
        items: Array.isArray(parsed.items)
            ? parsed.items.filter((it) => it.name).map((it) => ({
                name: String(it.name).trim(),
                qty: typeof it.qty === "number" ? it.qty : 0,
...
function parsePermitText(text) {
    ...
    if (items.length === 0)
        warnings.push("لم يتم التعرف على أي منتجات — راجع النص اليدوي");
    return { items, orderDate, deliveryDate, permitNumber, errors, warnings };
}
// ── OCR.space Provider (LAST RESORT — external) ─────────────────────────────
async function tryOCRSpace(imageBase64, mimeType) {
...
async function tryTesseract(imageBase64, mimeType) {
    console.log("[Tesseract] Trying local Tesseract OCR...");
    const tmpFile = (0, path_1.join)(process.env.TEMP || "/tmp", `scan_${Date.now()}.png`);
    const outFile = tmpFile.replace(".png", "");
    try {
        const buffer = Buffer.from(imageBase64, "base64");
        (0, fs_1.writeFileSync)(tmpFile, buffer);
        ...
        for (const cmd of tries) {
            try {
                ...
                const text = (0, fs_1.readFileSync)(outFileTxt, "utf-8");
        ...
        if (!rawText.trim()) {
            ...
            try {
                (0, child_process_1.execSync)(`tesseract "${tmpFile}" "${outFile}" -l eng --psm 4 --oem 3`, { timeout: 30000, stdio: "pipe" });
                rawText = (0, fs_1.readFileSync)(`${outFile}.txt`, "utf-8");
            }
            catch { }
        }
        ...
        try {
            (0, fs_1.unlinkSync)(`${outFile}.txt`);
        }
        catch { }
        try {
            (0, fs_1.unlinkSync)(`${outFile}_ara.txt`);
        }
        catch { }
    }
}
